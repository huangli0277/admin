...
class FinanceController extends AdminController
{
	...
	public function mytxmx($field = NULL, $name = NULL, $stu = NULL)
	{
		//$sql_Allbegin==M()->query("SELECT * FROM  a_ctc order by cid asc limit 1");
		//print_r($sql_Allbegin);
		//echo $Allbegin=$sql_Allbegin[0]['uptime'];
		$begintime=strtotime('00:00:00');
		$endtime=strtotime('23:59:59');
		$time=time();
		$daynum=floor(($time-1520683604)/86400);
		if($daynum>30)$daynum=30;
		for ($x=0; $x<=$daynum; $x++) {
			$chongzhi=array();
			$begintime=$begintime-86400;
			$endtime=$endtime-86400;
			$sql_cz=M()->query("SELECT sum(pri) as sum FROM  a_ctc where type = 2 and stu='2' and uptime > '$begintime' and uptime < '$endtime'");
			$chongzhi[0]['num']=$sql_cz[0]['sum'];
			$chongzhi[0]['title']="总量";
			$chongzhi[0]['daytime']=date("Y-m-d",$begintime);
			$arrName=array(1=>'陆日森',2=>'杨茜',3=>'张翔',4=>'张琪',5=>'张秋平');
			foreach($arrName as $k =>$v){
				$sql_ze=M()->query("SELECT sum(a.pri) as sum  FROM a_ctc a, qq3479015851_mycz_type b where a.tid=b.id and a.type='2' and a.stu='2'  and b.truename='".$v."' and uptime > '$begintime' and uptime < '$endtime'  order by a.cid asc ");
				//print_r($sql_ze);
				$chongzhi[$k]['title']=$v;
				if($sql_ze[0]['sum']){
					$chongzhi[$k]['num']=$sql_ze[0]['sum'];
				}else{
					$chongzhi[$k]['num']=0;
				}
			}	
	...
	{
		if($id>0){
			$mycz = M()->query("SELECT * FROM  a_ctc where  cid = '$id' and stu = 1 LIMIT 1; ");
			$userid = $mycz[0]['uid'];
			if($mycz){
				if($mycz[0]['type']==1){
	...
	public function myczmx($field = NULL, $name = NULL, $stu = NULL)
	{
		//$sql_Allbegin==M()->query("SELECT * FROM  a_ctc order by cid asc limit 1");
		//print_r($sql_Allbegin);
		//echo $Allbegin=$sql_Allbegin[0]['uptime'];
		...
		$endtime=strtotime('23:59:59');
		$time=time();
		$daynum=floor(($time-1520562950)/86400);
		if($daynum>31)$daynum=31;
		for ($x=0; $x<$daynum; $x++) {
			$chongzhi=array();
			$begintime=$begintime-86400;
			$endtime=$endtime-86400;
			$sql_cz=M()->query("SELECT sum(num) as sum FROM  a_ctc where type = '1' and stu='2'  and uptime > '$begintime' and uptime < '$endtime'");
			$chongzhi[0]['num']=$sql_cz[0]['sum'];
			$chongzhi[0]['title']="总量";
			$chongzhi[0]['daytime']=date('Y-m-d',$begintime);
			$arrName=array(1=>'陆日森',2=>'杨茜',3=>'张翔',4=>'张琪',5=>'张秋平');
			foreach($arrName as $k =>$v){
				$sql_ze=M()->query("SELECT sum(a.num) as sum  FROM a_ctc a, qq3479015851_mycz_type b where a.tid=b.id and a.type='1' and a.stu='2'  and b.truename='".$v."' and uptime > '$begintime' and uptime < '$endtime'  order by a.cid asc ");
				//print_r($sql_ze);
				$chongzhi[$k]['title']=$v;
				if($sql_ze[0]['sum']){
					$chongzhi[$k]['num']=$sql_ze[0]['sum'];
	...
	{
		$mycz = M()->query("SELECT * FROM  a_ctc where  cid = '$id' and stu = 1; ");
		if($mycz){
			if($mycz[0]['type']==1){
				M()->execute("UPDATE  `a_ctc` SET  `typer` = 2,stu=2 WHERE `cid` ='$id';");
				$num = $mycz[0]['num'];
				$userid = $mycz[0]['uid'];
				M()->execute("UPDATE  `qq3479015851_user_coin` SET  `cny` =  cny+$num WHERE `userid` ='$userid';");
			}else{
				$num = $mycz[0]['num'];
				$uid = $mycz[0]['uid'];
				M()->execute("UPDATE  `qq3479015851_user_coin` SET  `cnyd` =  cnyd-$num WHERE `userid` ='$uid';");
	...
	public function mycz($field = NULL, $name = NULL, $stu = NULL)
	{

		
		if($name) $where = " and ".$field."  = '$name' ";
		if($field=="moble"){
			$uid = M('User')->where(array('moble' => $name))->getField('id');
			 $where = " and uid  = '$uid' ";
		}
		if($stu){
			if($stu==3){
		...
		$cont_num = M()->query("SELECT count(cid) as count FROM  a_ctc where type = 1 group by uid  ");
		$cont['num'] = $cont_num[0]['count'];
		$cont_pri = M()->query("SELECT sum(pri) as pri FROM  a_ctc where stu = 2 and  type = 1 ");
		$cont['pri'] = $cont_pri[0]['pri'];
		$this->assign('cont', $cont);
		
		$count = M()->query("SELECT count(cid) as count FROM  a_ctc where type = 1 $where ");
		$count = $count[0]['count'];
		$Page = new \Think\Page($count, 15);
		$show = $Page->show();
		$list = M()->query("SELECT * FROM  a_ctc where type = 1 $where order by cid desc LIMIT $Page->firstRow , $Page->listRows; ");

		foreach ($list as $k => $v) {
			$type = M()->query("SELECT kaihu,truename,username,min FROM  qq3479015851_mycz_type where id = '".$v['tid']."' ");
			$list[$k]['kaihu'] = $type[0]['kaihu'];
			$list[$k]['truename'] = $type[0]['truename'];
			$list[$k]['username'] = $type[0]['username'];
			$list[$k]['min'] = $type[0]['min'];
			$list[$k]['moble'] = M('User')->where(array('id' => $v['uid']))->getField('moble');
			$list[$k]['name'] = M('User')->where(array('id' => $v['uid']))->getField('truename');
			$chong = $ti = $tui = $tuir =$wall =$wd = $rmb = $fen = $fens = 0;
			$chong = M()->query("SELECT sum(num) as sum FROM  `a_ctc` where uid ='".$v['uid']."' and type = 1 and stu = 2 ");
			$list[$k]['chong'] = $chong[0]['sum'];
			$ti = M()->query("SELECT sum(num) as sum FROM  `a_ctc` where uid ='".$v['uid']."' and type = 2 and stu = 2 ");
			$list[$k]['ti'] = $ti[0]['sum'];
			if($v['type']==1){
				$list[$k]['tpl'] = '买入';
	...
	public function mycz_qs($field = NULL, $name = NULL, $stu = NULL)
	{
		$list = M()->query("SELECT * FROM  a_ctc  order by cid desc LIMIT 0 , 1; ");
		if($list){
			$this->success($list[0]['cid']);
		}
	...
	public function mytx($field = NULL, $name = NULL, $stu = NULL)
	{
		if($name) $where = " and ".$field."  = '$name' ";
		if($field=="moble"){
			$uid = M('User')->where(array('moble' => $name))->getField('id');
		...
		$cont_num = M()->query("SELECT count(cid) as count FROM  a_ctc where type = 2 group by uid  ");
		$cont['num'] = $cont_num[0]['count'];
		$cont_pri = M()->query("SELECT sum(pri) as pri FROM  a_ctc where stu = 2 and  type = 2 ");
		$cont['pri'] = $cont_pri[0]['pri'];
		$this->assign('cont', $cont);
		
		$count = M()->query("SELECT count(cid) as count FROM  a_ctc where type = 2 $where ");
		$count = $count[0]['count'];
		$Page = new \Think\Page($count, 15);
		$show = $Page->show();
		$list = M()->query("SELECT * FROM  a_ctc where type = 2 $where order by cid desc LIMIT $Page->firstRow , $Page->listRows; ");
		...
		foreach ($list as $k => $v) {
			...
			$list[$k]['truename'] = $type[0]['truename'];
			$list[$k]['username'] = $type[0]['username'];
			$type1= M()->query("SELECT * FROM  qq3479015851_user_bank where userid = '".$v['uid']."' ");
			foreach($type1 as $kk => $vv){
				if($vv['backimg']) $type1[$kk]['img'] = $vv['imgm'] ? WAP_URL."/Upload/pay/".$vv['backimg'] : PC_URL."/Upload/pay/".$vv['backimg'];
			}
			$list[$k]['zhuans'] = $type1;
			$list[$k]['min'] = $type[0]['min'];
			$list[$k]['moble'] = M('User')->where(array('id' => $v['uid']))->getField('moble');
