...
class TradeController extends AdminController
{
	...
	public function log($field = NULL, $name = NULL, $market = NULL)
	{
		...
		if ($field && $name) {
			...
			else if ($field == 'userfone') {
				$where['userid'] = M('User')->where(array('moble' => $name))->getField('id');
			}
			else if ($field == 'peername') {
				$where['peerid'] = M('User')->where(array('username' => $name))->getField('id');
			}
			else if ($field == 'peerfone') {
				$where['peerid'] = M('User')->where(array('moble' => $name))->getField('id');
			}
			else {
				$where[$field] = $name;
			}
	...
	public function hdjl($field = NULL, $name = NULL)
	{
		$where = array();

		if ($field && $name) {
			if ($field == 'username') {
				$where['userid'] = M('User')->where(array('username' => trim($name) ))->getField('id');
			}elseif($field == 'hdid'){
				$where['hdid'] = M('ASignRule')->where(array('shuoming' => trim($name) ))->getField('id');
			}else {
				$where[$field] = $name;
			}
		}
		...
		$count = M('ASign')->where($where)->count();
		$Page = new \Think\Page($count, 15);
		$show = $Page->show();
		$list = M('ASign')->where($where)->order('id desc')->limit($Page->firstRow . ',' . $Page->listRows)->select();
		$laiyuan=array(0 =>'自己',1 =>'一级',2 =>'二级');
		foreach ($list as $k => $v) {
			$list[$k]['username'] = M('User')->where(array('id' => $v['userid']))->getField('username');
			$list[$k]['hdid'] = M('ASignRule')->where(array('id' => $v['hdid']))->getField('shuoming');
			$list[$k]['iftjr'] = $laiyuan[$v['iftjr']];
		}
