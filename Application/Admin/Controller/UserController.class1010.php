			...
			if($status>2){
				switch($status){
					...
					case "6":
						...
						break;
					case "9":
						$where = $where." and `idcardauth`=0 and idcardimg1 !='' ";
						break;
				}
		...
		if (empty($_POST)) {
			...
			if($user['idcardimg1']){
				...
				foreach($img_arr as $k1=>$v1){
					$imgstr1 = $imgstr1.'<img src="http://m.jy.hltxgw.cn/Upload/idcard/'.$v1.'"  style="width:200px;height:100px;" />';
				}
		...
		case 'idauth': 
			...
			foreach($dss as $k=>$v){
				...
				if(!$isnrt[0]['isnrt']){
					...
					M()->execute("UPDATE `qq3479015851_user_coin` SET  `nrt` =  nrt+100 WHERE userid ='".$v."';");
				}
				//自己推荐赠送50
				...
				if($istj[0]['coun'] > $isnrt[0]['isnrts']){
					...
					if($num){
						...
						M()->execute("UPDATE `qq3479015851_user_coin` SET  `nrt` =  nrt+$pri WHERE userid ='".$v."';");
					}
				}
			//更新推荐人的赠送
				if($isnrt[0]['invit_1']){
					...
					if($isnrt_tj){
						...
						if($istj[0]['coun'] > $isnrt_tj[0]['isnrts']){
							...
							if($num){
								...
								M()->execute("UPDATE `qq3479015851_user_coin` SET  `nrt` =  nrt+$pri WHERE userid ='".$tjid."';");
							}
						}
					}
				}
