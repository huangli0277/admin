...
class IndexController extends AdminController
{
	public function index()
	{
		...
		$arr['cny_num'] = M('UserCoin')->sum('cny') ;
		$arr['cny_num'] = round($arr['cny_num'],2);
		$arr['cny_numd'] = M('UserCoin')->sum('cnyd') ;
		$arr['cny_numd'] = round($arr['cny_numd'],2);
		...
		for (; $i < 30; $i++) {
			...
			$mycz =M()->table("a_ctc")->where(array(
				'type'  => 1,
				'stu'  => 2,
				'uptime' => array(
					array('gt', $a),
			...
			$mytx = M()->table("a_ctc")->where(array(
				'type'  => 2,
				'stu'  => 2,
				'uptime' => array(
					array('gt', $a),
	...
	public function coin($coinname = NULL)
	{
		...
		$this->assign('cztx', json_encode($cztx1));
		$this->display();
	}
