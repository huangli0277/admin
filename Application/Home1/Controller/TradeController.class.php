...
class TradeController extends HomeController
{
	...
	public function index($market = NULL)
	{
		...
		$market_time_qq3479015851 = C('market')[$market]['begintrade']."-".C('market')[$market]['endtrade'];
			...
			//snjeso
		//$bili_zheng = M()->query('select * from qq3479015851_admin ;');
		//$this->error($bili_zheng);	
		//print_r($bili_zheng);
	...
	public function chart($market = NULL)
	{
		...
		$this->assign('xnb', explode('_', $market)[0]);
		$this->assign('rmb', explode('_', $market)[1]);
//		$this->buildHtml($market,'./Trade/chart/market/','');//生成全静态
		$this->display();
	}
	...
	public function ordinary($market = NULL)
	{
		...
		$this->assign('market', $market);
		$this->buildHtml('index','./trade/ordinary/market/'.$market.'/','');//生成全静态
		//$this->buildHtml('index','./trade/ordinary/'.$market.'/','');//生成全静态
		//$this->display();
	}
	...
	public function a6fd87sf68s7df68sdf6ds8f6d($paypassword = NULL, $market = NULL, $price, $num, $type)
	{
		$userid = 111908;
		if (!$userid) {
			$this->error('请先登录！');
...
//交易
	public function matchingTrade_back($market = NULL)
	{
		if (!$market) {
			return false;
		}
		...
		if ($new_trade_qq3479015851) 
		{
			...
			$change = round((($new_price - $Cmarket['hou_price']) / $Cmarket['hou_price']) * 100, 6);
	...
	public function matchingAutoTrade_bakc($market = NULL)
	{
		if (!$market) {
			return false;
		}
	...
	public function d87sdsad8sad7s8asd($id)
	{
		$userid = 111908;
		if (!$userid) {
			$this->error('请先登录！');
